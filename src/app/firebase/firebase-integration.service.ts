...
export class FirebaseService {
  ...
  */
  ...
  private getUser(userId: string): Observable<FirebaseUserModel> {
    ...
    .pipe(
      map(a => {
        ...
        const id = a.payload.id;
        const age = this.calcUserAge(userData.birthdate);
  ...
  /* 
    Auth functions
    Added by Phan Duc Dung
    Added date: 2019-10-21
  */
  public doLoginFb() {
  ...
  /*
    Firebase Family data
  */
  // Get Family data from user
  public getFamilyTree() {
    return new Promise<any>((resolve, reject) => {
      let currentUser = this.getCurrentUser();
      if(currentUser){
        this.snapshotChangesSubscription = this.afs.collection('familytree').doc(currentUser.uid).valueChanges()
        .subscribe(snapshots => {
          resolve(snapshots);
        }, err => {
          reject(err)
  ...
  public getFamilyTreeTest(): Observable<JsonGedcomData> {
    let currentUser = this.getCurrentUser();
    return this.afs.doc<JsonGedcomData>('familytree/' + currentUser.uid)
    .snapshotChanges()
    .pipe(
      map(a => {
        const data = a.payload.data();
        const id = a.payload.id;
        return { id, ...data } as JsonGedcomData;
      })
    );
  }

  public createFamilyTree(familytree: JsonGedcomData) {
    return new Promise<any>((resolve, reject) => {
      let currentUser = firebase.auth().currentUser;
      this.afs.collection('familytree').doc(currentUser.uid).set({...familytree})
      .then(
        res => resolve(res),
        err => reject(err)
      )
